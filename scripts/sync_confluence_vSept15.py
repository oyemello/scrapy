#!/usr/bin/env python3
"""
Confluence → MkDocs (vSept15)
Includes link-following up to 2 levels for in-page Confluence links.
"""
import os
import re
import sys
...
class Config:
    ...
    # Link following controls (default 2 levels)
    ...
    def from_env(cls) -> "Config":
        ...
        if d := os.getenv('DOCS_DIR'):
            data['docs_dir'] = Path(d)
        if m := os.getenv('MKDOCS_PATH'):
            data['mkdocs_path'] = Path(m)
        # Optional link following overrides
...
class Processor:
    ...
    def collect(self, root_id: str) -> Dict[str, Page]:
        """Collect pages; follow in-page links up to cfg.max_link_depth."""
    ...
    def to_markdown(self, html: str) -> str:
        return md(html or '', heading_style='ATX', strip=None)
    ...
    def _write_homepage(self, pages: Dict[str, Page], fmap: Dict[str, Path], root_id: str) -> None:
        ...
        if root.children and root_id in fmap:
            ...
            tiles.append(("<a class=\"category-card\" href=\"{href}\">"
                          ...
                          "</a>").format(href=root_href, title=htmlesc.escape(site_title)))
        for cid in root.children:
            ...
            tiles.append(("<a class=\"category-card\" href=\"{href}\">"
                          "  <div class=\"card-title\">{title}</div>"
                          "</a>").format(href=href, title=htmlesc.escape(title)))
        grid_html = "\n".join(tiles)
        homepage_md = (f"# {site_title}\n\n"
